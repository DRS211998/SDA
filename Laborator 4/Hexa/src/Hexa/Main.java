...
public class Main {
	...
	public static void main(String[] args) {
		...
		String stringArray[] = new String[shapes.size()];//adauga culoriile
		int z=0;
		for(Shape s: shapes)
		{
		...
		for(int i=0; i<stringArray.length; i++)//de la 0 la z de sus
		{
	...
	static int countOccurrences(String arr[], int n, String x) //functia pt count
	{ 
	    int res = 0; 
	    for (int i=0; i<n; i++) 
	        if (x == arr[i]) 
